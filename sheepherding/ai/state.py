...
class State:
    ...
    def prefeatures(self, dog):
        ...
        feasible_sheep = [(sheep_ix, sheep) for sheep_ix, sheep in enumerate(dog.world.sheeps) if not is_sheep_in_target(sheep, self.target_location, self.target_radius)]
        if len(feasible_sheep) == 0:
            feasible_sheep = enumerate(dog.world.sheeps)
        # sort based on distance to dog
        sorted_sheep = sorted([(dog.loc.distance(sheep.loc), sheep_ix) for sheep_ix, sheep in feasible_sheep])
